...
public class UserService : IUserService
{
    ...
    private readonly IMultiTenantContextAccessor<ABCSchoolTenantInfo> _tenantContextAcessor;

    public UserService(
        UserManager<ApplicationUser> userManager,
        RoleManager<ApplicationRole> roleManager,
        ApplicationDbContext dbContext,
        IMultiTenantContextAccessor<ABCSchoolTenantInfo> tenantContextAcessor)
    {
        _userManager = userManager;
        _roleManager = roleManager;
        _dbContext = dbContext;
        _tenantContextAcessor = tenantContextAcessor;
    }
    ...
    public async Task<string> AssignRolesAsync(string userId, UserRolesRequest request)
    {
            ...
            && request.UserRoles.Any(ur => !ur.IsAssigned && ur.Name == RoleConstants.Admin))
        {
            ...
            {
                if (_tenantContextAcessor.MultiTenantContext.TenantInfo.Id == TenancyConstants.Root.Id)
    ...
    {
        var userInDb = await GetUserAsync(userId);
        if (userInDb.Email.Equals(TenancyConstants.Root.Email))
            throw new ConflictException(["Not allowed to remove Admin User from a Root Tenant"]);
    ...
    public async Task<List<string>> GetUserPermissionsAsync(string userId, CancellationToken ct)
    {
        ...
        foreach (var role in
            ...
            .AsNoTracking()
            .Where(r => userRolesNames.Contains(r.Name)).ToListAsync(ct))
        {
            permissions.AddRange(await _dbContext
                .RoleClaims
    ...
    public async Task<List<UserRoleResponse>> GetUserRolesAsync(string userId, CancellationToken ct)
    {
        ...
        var rolesInDb = await _roleManager.Roles.AsNoTracking().ToListAsync(ct);
